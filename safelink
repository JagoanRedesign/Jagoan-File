/*	
Author : Ilham Galeh P	
Get Lisensi Now : https://www.jagoanredesign.eu.org
Versi : Premium	
*/
...
if(tLink==window.location.origin+"/"){
  const timeSafe=15000;
      ...
      xhr.onreadystatechange = function() {
          if (xhr.readyState == 4) {
              if(xhr.status == 200) {
                  ...
                  let data=obj.data;
                  let hash=obj.hash;
                  var textButton=obj.textButton;
                  if(textButton.split("").length==0){
                      textButton="Get Link"
                  };
                  var statusSafe="";
                  if(data.split("").length>0 && hash.split("").length>0){
                      statusSafe=true;
                  };
                  if(statusSafe==true){
                      let dbSafe=aesCrypto.decrypt(data,hash);
                      document.getElementById("safe").innerHTML=`<div><button id="dbGetlink">`+textButton+`</button></div>`;
                      document.getElementById("dbGetlink").addEventListener("click",function(){
                          document.getElementById("dbGetlink").setAttribute("disabled","true");
                          timeRun(data,hash,textButton);
                      })
                  };
               };
          };
  ...
  var countTime=0;
  function timeRun(data,hash,titleText){
      if(countTime<=timeSafe){
          document.getElementById("dbGetlink").innerHTML=`Please Wait.. `+Math.round((countTime/timeSafe)*100) +" %";
          setTimeout(function(){
              countTime=countTime+100;
              timeRun(data,hash,titleText);
          },100);
      } else {
          //document.getElementById("dbGetlink").removeAttribute("disabled");
          document.getElementById("dbGetlink").innerHTML="Geting Request Your Link";
          //document.getElementById("dbGetlink").setAttribute("onclick",`getLink('`+data+`','`+hash+`')`);
			getLink(data,hash,titleText);
      };
  };
  function getLink(data,hash,titleText){
      //document.getElementById("dbGetlink").setAttribute("disabled","true");
    setTimeout(function(){
     $.ajax({
            url: window.location.origin + "/feeds/posts/summary/?alt=json-in-script&orderby=updated&max-results=9999",
            type: 'get',
            dataType: 'jsonp',
            success: function aku(younglex) {
                ...
                hasilgetlinkarp = listlinkku[parseInt(Math.random() * listlinkku.length)];
                localStorage.setItem("linkSafe",aesCrypto.decrypt(data,hash));
				localStorage.setItem("titleText",titleText);
				document.getElementById("safe").innerHTML=`<a href="`+hasilgetlinkarp+`">VISIT LINK</a>`;
            },
            async: false
        });
    },500);
	...
	if(localStorage.getItem("linkSafe")==null==false){
   			document.getElementById("safe").innerHTML=`<button id="dbGetlink">`+localStorage.getItem("titleText")+`</button>`;
			const linkSafe=localStorage.getItem("linkSafe");
			localStorage.removeItem("linkSafe");
			document.getElementById("safe").addEventListener("click",function(){
              document.getElementById("safe").innerHTML=`<div class="areaScrollNotif"><span class="notifScroll">The link is below...<br/>Please Scroll Down...</span><br/><svg style="width:24px;height:24px" viewBox="0 0 24 24">
   ...
   document.getElementById("endSafe").innerHTML=`<div><div class="titleSafe">The link is here</div><div><input id="dbLinkSafe" disabled="true" value="`+linkSafe+`"/></div><div><button onclick="copyLinkSafe()" class="copySafe">Copy</button></div></div>`;

		});
	};
	function copyLinkSafe(){
		document.getElementById("dbLinkSafe").removeAttribute("disabled");
		document.getElementById("dbLinkSafe").select();
		document.execCommand("copy");
		document.getElementById("dbLinkSafe").setAttribute("disabled","true");
	};
};
